...
adjective band~\cite{brooke1996sus,bangor2009determining}. Distributions,
efficiency ratios, and SUS scores are computed at display time from the
stored task results; per-task error, hesitation, and action counts are
persisted when the task completes, alongside the individual timestamped
log entries they summarize.
...
comparison---several evaluators logging the same scripted session with
...
logging error. The \emph{platform} track ships features the studies
motivated: temporal views that plot the already-timestamped error and
hesitation events along each task's timeline, raw CSV/JSON export for
external statistical analysis, a more flexible task model,
\emph{retroactive} anonymization of already-collected sessions
(complementing the prospective identity-field omission the platform
ships today), multi-evaluator observation of a single live session, and
localization of the participant client, whose users in both studies
preferred answering in their native language.

\section*{Availability}
The platform is deployed at \url{https://avalux.pt}. Its source---the
application, the database schema (31 SQL migrations), the seeded protocol
of study~2, the in-application tutorial, and the report generator---is
available at \textbf{[repository URL and license to be inserted before
submission]}. Anonymized session data from the case studies is available
on request within the limits of the participants' consent.
