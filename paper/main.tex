...
\begin{itemize}
  ...
  (mean SUS 28.1 vs.\ 80);
  \item a \textbf{pedagogical design} for classroom use: features
  mapped onto learning objectives, student teams running independent
  studies under an instructor account, and a protocol review that
  flags common novice mistakes before a study runs.
\end{itemize}
...
measurable per session rather than assumed. When a
...
designing~\cite{churchill2013teaching}. A systematic mapping of HCI
teaching found active, project-based approaches dominant, only two
tools built for HCI teaching, and few empirical evaluations of
either~\cite{lima2019tools}. What novices actually find hard has been
...
own assumption about what is hard. A tool that makes an expert faster does
not thereby make a novice better; the education literature separates
\emph{productivity tools} from \emph{mindtools}, which engage the
learner in thinking about the domain~\cite{jonassen2000mindtools}. Our claim is of the second kind
and rests on the cognitive-apprenticeship
observation~\cite{collins1989cognitive} that expert practice is
invisible to novices until made visible, scaffolded, and reflected
on. With a stopwatch and a spreadsheet an instructor sees a
final report and cannot tell whether moderation was biased, whether two
observers would have logged the same errors, or whether the sample
supports the conclusion. Avalux makes that methodological quality
observable, to student and instructor alike. Table~\ref{tab:pedagogy}
states the constructive alignment~\cite{biggs1996alignment} between
each learning objective, what the platform offers toward it, and the
evidence that would show it met; that third column is also the rubric
of the classroom study in Section~\ref{sec:future}. Co-rating with Cohen's $\kappa$ turns ``what counts as an
error'' from a textbook warning into a number two students computed
about their own disagreement, and SUS confidence intervals refuse a
score without its uncertainty at the moment $n=5$ would be over-read.
That disagreement is not a student failing. Reviewing eleven studies of
cognitive walkthrough, heuristic evaluation, and thinking-aloud, Hertzum
and Jacobsen report that average agreement between any two evaluators of
the same system ranges from 5\% to 65\%, and that the effect holds for
experienced and novice evaluators alike and for severity ratings as well as
problem detection~\cite{hertzum2003evaluator}. They attribute it to vague
goal analyses, vague evaluation procedures, and vague problem criteria:
precisely the three things a protocol template is forced to make explicit
before a session runs. Measuring $\kappa$ therefore does more than grade a
pair of students; it exposes a property of the method itself.

These features observe; the protocol review is the first that gives
feedback, checking a template without blocking against common novice
mistakes (tasks that name interface labels and so lead the participant,
tasks without a success criterion or optimal path, no practice task,
no error taxonomy, no post-session instrument), each with a rationale
linked to the tutorial. Existing support for student studies addresses
planning: a toolkit for method selection, power, and
counterbalancing~\cite{schwind2023toolkit}, and a widely used plan
template whose assessment turns on an instructor gate before any
session may run~\cite{rose2016plan}. Avalux begins where they stop, at
the session itself. Its own gate, enabled per template at an
organization's discretion, is deliberately narrower than that template's.
Schwind et al.\ warn that close supervision calls into question the
independence of a student's research work~\cite{schwind2023toolkit}, so
approval gates \emph{recruiting} and not \emph{rehearsal}: a team may run
its protocol before approval, and those sessions are recorded as pilots and
excluded from the template's aggregates. The instructor thus reviews with
the protocol review's findings and the team's own pilot data in hand,
without having authored the protocol. Its review appears only once a design is saved,
not while it is typed: students value procedural scaffolding but
distrust it when it pre-empts their own judgment~\cite{aziz2026genai},
and a deterministic check carries none of the sycophancy they report
in generative facilitators. Feedback is what separates a pedagogical
tool from a well-instrumented one.

Two limits of this coverage should be stated plainly. The platform acts on
difficulties of method execution and interpretation, not on difficulties of
design itself (\textsc{what}, \textsc{adapt}, \textsc{divrs}) or of
teamwork, scope, and stakeholders. More consequentially, it offers no
inspection method. Heuristic evaluation and walkthroughs ordinarily precede
empirical testing in a course sequence, need neither participants nor
ethical approval, and produce the ranked problem list that a usability test
is then designed to confirm; Avalux presently begins at the second stage of
that pipeline. What remains missing, and the classroom evaluation this
framing requires, are taken up in Section~\ref{sec:future}.
...
\begin{tabular}{@{}p{0.30\columnwidth}p{0.30\columnwidth}p{0.31\columnwidth}@{}}
\toprule
...
Know when a protocol is ready to run (\textsc{stage}, \textsc{rush}) & Optional instructor gate; unapproved runs recorded as pilots & Approval recorded before the first recruited participant \\
...
established. SEQ and SUS are self-report instruments with
...
The pedagogical framing of Section~III adds a classroom study: two
...
methodological understanding, the protocol review's findings showing
which mistakes recur; it is the kind of rigorous evaluation the mapping
of HCI teaching found largely absent~\cite{lima2019tools}. The rest of the loop is still missing, at both ends. Before a test is
designed, the platform offers no inspection method: independent heuristic
evaluation of a team's own prototype or of comparable products, merged into
a ranked problem list from which test tasks are derived, would both supply
that missing first stage and let a cohort observe the evaluator effect
directly, with no participants required. After a session, a reflection
prompt, structured instructor review visible to the team, and comparison of
one protocol across teams or design iterations remain to be built.
...
application, 49 SQL migrations, the seeded protocol of study~2, the
