...
either~\cite{lima2019tools}. A tool that makes an expert faster does
...
the session itself. Its review appears only once a design is saved,
...
tool from a well-instrumented one; what remains missing, and the
classroom evaluation this framing requires, are taken up in
Section~\ref{sec:future}.

\begin{table}[t]
\caption{Constructive alignment: each learning objective, what the
platform offers toward it, and the evidence that it was met.}
\label{tab:pedagogy}
\centering
\footnotesize
...
\midrule
Write tasks as goals, not procedures & Protocol review, leading-task check & Warnings resolved before the first session \\
Fix criteria and baselines before running & Success criterion and optimal path per task & Both present on every measured task \\
Control order effects & Order strategy per session, with explainer & Strategy chosen and justified in the report \\
Treat ``error'' as a judgement & Co-rating, Cohen's $\kappa$ & $\kappa$ reported; disagreements reconciled \\
Report uncertainty with small $n$ & SUS with 95\% CI & An interval beside every score \\
Moderate without leading & Observation view, timestamped notes & Peer notes on hints and interventions \\
Analyse qualitatively as a procedure & Code book, frequency matrix & Codes defined before tagging; matrix reported \\
Handle participant data ethically & Retroactive anonymization, private media; consent is recorded outside the platform & Anonymized export; consent on file for every participant \\
\bottomrule
\end{tabular}
\end{table}
...
comparison is the designed next study (Section~\ref{sec:future}).
...
of HCI teaching found largely absent~\cite{lima2019tools}. The rest of the feedback loop is still missing: a reflection prompt after each
session, structured instructor review visible to the team, and
comparison of one protocol across teams or design iterations.
