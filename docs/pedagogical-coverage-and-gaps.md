...
| L2 | Computing students hit **18 distinct kinds of difficulty** learning HCI. Several are method difficulties a tool can act on: `WHY` "why do we do this activity this way", `HOW` "how do I perform this method", `SYNTH` "how do I interpret this feedback", `BIAS` "how do I avoid biasing my design", `STAGE` "when should I move to the next design stage", plus the educator-reported `RUSH` "students rush to implement and discount design work". (`EVAL` is *not* one of these: verified against the source, it means "how can I choose between options", a design-choice difficulty, not an evaluation one.) | Oleson, Solomon and Ko, CHI 2020 | [read], codes re-verified against the source |
